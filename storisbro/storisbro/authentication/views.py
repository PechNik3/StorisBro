...
class VKAuthView(View):
    ...
    def get(self, request):
        """Redirect to VK login."""
        redirect_url = settings.SOCIAL_AUTH_VK_REDIRECT
    ...
    def get(self, request):
        ...
        response = requests.post(
            ...
            params={
                ...
                'redirect_uri': settings.SOCIAL_AUTH_VK_REDIRECT,
                'code': code,
            }
        )
        ...
        if response.status_code == HTTPStatus.OK:
            ...
            if user_info_response.status_code == HTTPStatus.OK:
                ...
                email = f'{username.lower().replace(" ", ".")}@example.com'  # Временный email

                # Пример возврата данных
                return HttpResponse(f'Username: {username}, Email: {email}')
        return HttpResponse('Ошибка авторизации через ВКонтакте', status=400)
